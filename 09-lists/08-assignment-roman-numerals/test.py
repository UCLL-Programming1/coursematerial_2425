...
@if_function_exists("to_roman")
@pytest.mark.parametrize("value, expected", enumerate(test_data, start=1))
def test_to_roman(value, expected):
...
def test_from_roman(numeral, expected):
    actual = student.from_roman(numeral)
    assert expected == actual, f"from_roman({numeral}) should equal {expected} but was {actual}"
